  ...
  group('MasonexGenerator', () {
    ...
    group('.fromBrick (path)', () {
      ...
      test('constructs an instance (nested conditional, true)', () async {
        ...
        final file = File(path.join(tempDir.path, 'brick_masonex.txt'));

        expect(file.existsSync(), isTrue);

        expect(file.readAsStringSync(), equals('Hello Brick Masonex'));
      });
    ...
    group('.fromBundle', () {
      test('constructs an instance', () async {
        const name = 'Dash';
        final generator = await MasonexGenerator.fromBundle(greetingBundle);
        final tempDir = Directory.systemTemp.createTempSync();
      ...
      test('constructs an instance with hooks', () async {
        const name = 'Dash';
        final generator = await MasonexGenerator.fromBundle(hooksBundle);
        final tempDir = Directory.systemTemp.createTempSync();
      ...
      test('constructs an instance (photos)', () async {
        final generator = await MasonexGenerator.fromBundle(photosBundle);
        final tempDir = Directory.systemTemp.createTempSync();
    ...
    group('compareTo', () {
      test('returns 0 when generators are the same type', () async {
        final generatorA = await MasonexGenerator.fromBundle(greetingBundle);
        final generatorB = await MasonexGenerator.fromBundle(greetingBundle);
        expect(generatorA.compareTo(generatorB), equals(0));
      });

      test('returns -1 when generators are different types', () async {
        final generatorA = await MasonexGenerator.fromBundle(greetingBundle);
        final generatorB = await MasonexGenerator.fromBundle(hooksBundle);
