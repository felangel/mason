  ...
  group(
    ...
    () {
      ...
      test('can be instantiated with existing bricks.json', () async {
        ...
        final bricksJson = BricksJson(directory: directory);
        File(
          path.join(directory.path, '.masonex', 'bricks.json'),
      ...
      group('add', () {
        ...
        test('adds bricks to bricks.json (git)', () async {
          ...
          expect(result.brick.name, equals(brick.name));
          expect(
            result.brick.location.git!.url,
            equals(brick.location.git!.url),
          );
          expect(
        ...
        test('adds bricks to bricks.json (git + ref) from remote', () async {
          ...
          expect(
            result.brick.location.git!.ref,
            equals(brick.location.git!.ref),
          );
        ...
        test(
            'throws BrickIncompatibleMasonexVersion '
            'when brick is incompatible with masonex version.', () async {
          final directory = Directory.systemTemp.createTempSync();
          final bricksJson = BricksJson(directory: directory);
          final file = File(
            path.join(directory.path, '.masonex', 'bricks.json'),
          ...
          final brickDirectory = Directory(path.join(directory.path, 'example'))
            ..createSync(recursive: true);
          File(path.join(brickDirectory.path, BrickYaml.file))
              .writeAsStringSync(
            '''
name: example
description: example
...
''',
          ...
          try {
            ...
            fail('should throw');
          } on BrickIncompatibleMasonexVersion catch (error) {
            expect(
              error.message,
              equals(
        ...
        test(
            'throws MasonexYamlNameMismatch when '
            'masonex.yaml contains mismatch (path)', () async {
          final directory = Directory.systemTemp.createTempSync();
          final bricksJson = BricksJson(directory: directory);
          final file = File(
            path.join(directory.path, '.masonex', 'bricks.json'),
          ...
          expect(
            ...
            throwsA(isA<MasonexYamlNameMismatch>()),
          );
        });
      ...
      group('rootDir', () {
        ...
        test('uses APPDATA on windows environment when APPDATA exists', () {
          ...
          final appDataCacheDirectory = Directory(
            path.join(directory.path, 'Masonex', 'Cache'),
          )..createSync(recursive: true);
