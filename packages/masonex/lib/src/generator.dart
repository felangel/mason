...
/// {@template masonex_generator}
/// A [MasonexGenerator] which extends [Generator] and
/// exposes the ability to create a [Generator] from a
/// [Brick].
/// {@endtemplate}
class MasonexGenerator extends Generator {
  /// {@macro masonex_generator}
  MasonexGenerator(
    String id,
    String description, {
  ...
  /// Factory which creates a [MasonexGenerator] based on
  /// a local [MasonexBundle].
  static Future<MasonexGenerator> fromBundle(MasonexBundle bundle) async {
    ...
    return MasonexGenerator._fromBrick(target.path);
  }

  /// Factory which creates a [MasonexGenerator] based on
  /// a [GitPath] for a remote [BrickYaml] file.
  static Future<MasonexGenerator> fromBrick(Brick brick) async {
    final path = brick.location.path != null
        ? brick.location.path!
        : (await BricksJson.temp().add(brick)).path;
    return MasonexGenerator._fromBrick(path);
  }

  static Future<MasonexGenerator> _fromBrick(String path) async {
