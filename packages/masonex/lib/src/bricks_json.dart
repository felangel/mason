...
/// {@endtemplate}
class BrickResolveVersionException extends MasonexException {
  /// {@macro brick_resolve_version_exception}
  const BrickResolveVersionException(super.message);
}

/// {@template masonex_yaml_name_mismatch}
/// Thrown when a brick's name in `masonex.yaml` does not match
/// the name in `brick.yaml`.
/// {@endtemplate}
class MasonexYamlNameMismatch extends MasonexException {
  /// {@macro masonex_yaml_name_mismatch}
  MasonexYamlNameMismatch(super.message);
}

/// {@template brick_unsatisfied_version_constraint}
/// Thrown when a brick version constraint could not be satisfied.
/// {@endtemplate}
class BrickUnsatisfiedVersionConstraint extends MasonexException {
  /// {@macro brick_unsatisfied_version_constraint}
  BrickUnsatisfiedVersionConstraint(super.message);
}

/// {@template malformed_bricks_json}
/// Thrown when a brick from masonex.yaml cannot be found in bricks.json.
/// {@endtemplate}
class MalformedBricksJson extends MasonexException {
  /// {@macro malformed_bricks_json}
  const MalformedBricksJson(super.message);
}

/// {@template brick_incompatible_masonex_version}
/// Thrown when the current masonex version is incompatible with the brick.
/// {@endtemplate}
class BrickIncompatibleMasonexVersion extends MasonexException {
  /// {@macro brick_incompatible_masonex_version}
  BrickIncompatibleMasonexVersion({
...
/// A local cache for masonex bricks.
///
...
class BricksJson {
  ...
  BricksJson({required Directory directory}) {
    _bricksJsonFile = File(p.join(directory.path, '.masonex', 'bricks.json'));
  ...
  /// Populates cache based on `.masonex/bricks.json`.
  ...
  CachedBrick _addLocalBrick(Brick brick) {
    ...
    if (yaml.name != name) {
      throw MasonexYamlNameMismatch(
        'Brick name "$name" '
        'doesn\'t match provided name "${yaml.name}" in ${MasonexYaml.file}.',
      );
    }

    _verifyMasonexVersionConstraint(yaml);
  ...
  Future<void> _download(Brick brick, Directory directory) async {
    ...
    final bundle = await MasonexBundle.fromUniversalBundle(response.bodyBytes);

    unpackBundle(bundle, directory);
  }

  /// Bundled subdirectory within the masonex cache.
  static Directory get bundled => Directory(p.join(rootDir.path, 'bundled'));

  /// Global subdirectory within the masonex cache.
  ...
  static Directory get rootDir {
    ...
    if (environment.containsKey('MASON_CACHE')) {
      ...
      final appDataCacheDir = Directory(p.join(appData, 'Masonex', 'Cache'));
      if (appDataCacheDir.existsSync()) return Directory(appDataCacheDir.path);
      final localAppData = environment['LOCALAPPDATA']!;
      return Directory(p.join(localAppData, 'Masonex', 'Cache'));
    } else {
      return Directory(p.join(environment['HOME']!, '.masonex-cache'));
    }
  }
}
...
void _verifyMasonexVersionConstraint(BrickYaml brickYaml) {
  if (!isBrickCompatibleWithMasonex(brickYaml)) {
    throw BrickIncompatibleMasonexVersion(
      brickName: brickYaml.name,
      constraint: brickYaml.environment.masonex,
    );
  }
}
