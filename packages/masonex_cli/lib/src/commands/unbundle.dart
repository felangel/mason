...
import 'package:masonex_cli/src/commands/bundle.dart';
import 'package:path/path.dart' as path;

/// {@template unbundle_command}
/// `masonex unbundle` command exposes the ability to generate brick templates
/// from bundles.
/// {@endtemplate}
class UnbundleCommand extends MasonexCommand {
  ...
  Future<int> run() async {
    ...
    try {
      late final MasonexBundle bundle;
      switch (bundleType) {
...
Future<MasonexBundle> _parseDartBundle(File bundleFile) async {
  final rawBundle = await bundleFile.readAsString();
  return MasonexBundle.fromDartBundle(rawBundle);
}

Future<MasonexBundle> _parseUniversalBundle(File bundleFile) async {
  final rawBundle = await bundleFile.readAsBytes();
  return MasonexBundle.fromUniversalBundle(rawBundle);
}
