...
enum BundleType {
  /// Bundle Type which is supported by masonex natively
  universal,
...
/// `masonex bundle` command exposes the ability to generate bundles
/// from brick templates.
/// {@endtemplate}
class BundleCommand extends MasonexCommand {
  ...
  Future<int> run() async {
    ...
    MasonexBundle? previousBundle;
    if (setExitIfChanged && bundleGenerator.bundleFile.existsSync()) {
      switch (bundleType) {
        case BundleType.dart:
          previousBundle = await MasonexBundle.fromDartBundle(
            await bundleGenerator.bundleFile.readAsString(),
          );
        case BundleType.universal:
          previousBundle = await MasonexBundle.fromUniversalBundle(
...
/// brick bundle for a given [MasonexBundle].
///
/// See also:
///
...
abstract class _BrickBundleGenerator {
  ...
  const _BrickBundleGenerator({
    ...
    required MasonexBundle bundle,
  }) : _bundle = bundle;

  /// The respective [MasonexBundle] for which a brick bundle will be generated.
  final MasonexBundle _bundle;

  /// The file that we will be writing the brick bundle to.
  final File bundleFile;

  /// Generates the brick bundle file for the given [MasonexBundle].
...
class _BrickDartBundleGenerator extends _BrickBundleGenerator {
  ...
  Future<void> generate() async {
    ...
    await bundleFile.writeAsString(
      "// GENERATED CODE - DO NOT MODIFY BY HAND\n// ignore_for_file: type=lint, implicit_dynamic_list_literal, implicit_dynamic_map_literal, inference_failure_on_collection_literal\n\nimport 'package:masonex/masonex.dart';\n\nfinal ${_bundle.name.camelCase}Bundle = MasonexBundle.fromJson(<String, dynamic>${json.encode(_bundle.toJson())});",
    );
  }
}
