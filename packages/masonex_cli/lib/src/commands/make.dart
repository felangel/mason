...
/// `masonex make` command which generates code based on a brick template.
/// {@endtemplate}
class MakeCommand extends MasonexCommand {
  /// {@macro make_command}
  ...
  Future<int> run() async {
    ...
    usageException(
      '''Could not find a subcommand named "$subcommand" for "masonex make".''',
    );
  }
}
...
class _MakeCommand extends MasonexCommand {
  ...
  @override
  Future<int> run() async {
    if (!isBrickCompatibleWithMasonex(_brick)) {
      logger.err(
        '''The current masonex version is $packageVersion.\nBecause $name requires masonex version ${_brick.environment.masonex}, version solving failed.''',
      );
    ...
    Future<int> _make() async {
      ...
      final generator = await MasonexGenerator.fromBrick(Brick.path(path));
